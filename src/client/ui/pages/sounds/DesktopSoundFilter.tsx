...
const DesktopSoundFilter: React.FC<DesktopSoundFilterProps> = ({
  ...
  const getFilterButtonClass = (
    ...
    let baseClasses =
      "flex flex-row px-3 py-1.5 text-2xs rounded-lg border transition-colors duration-150 ";
    ...
    // If filter is actually in use
    if (
      (typeof selectedValue === "string" && selectedValue) ||
      (Array.isArray(selectedValue) && selectedValue.length > 0) ||
      (name === "Tempo" && (minTempo > 40 || maxTempo < 200))
    ) {
      baseClasses += "border-blue-500 ";
    }

    // Text color
    baseClasses += "bg-transparent text-neutral-600 dark:text-neutral-300";
  ...
  const renderFilterLabel = (filterName: string): string => {
    switch (filterName) {
      case "Tempo":
        return minTempo > 40 || maxTempo < 200 ? `${minTempo} - ${maxTempo}` : "";
  
      case "Key":
        return selectedKeys ? `${selectedKeys} ${selectedScale.toLowerCase().substring(0, 3)}` : "";
  
      case "Genre":
        return selectedGenres.length > 0 ? selectedGenres.join(", ") : "";
  
      case "Artist":
        return selectedArtists.length > 0 ? selectedArtists.join(", ") : "";
  
      case "Instrument":
        return selectedInstruments.length > 0 ? selectedInstruments.join(", ") : "";
  
      case "Mood":
        return selectedMoods.length > 0 ? selectedMoods.join(", ") : "";
  
      case "Keyword":
        return selectedKeywords.length > 0 ? selectedKeywords.join(", ") : "";
  ...
  return (
    ...
    <div className="hidden md:block w-full border-neutral-600 bg-white dark:bg-black/50 sticky top-0 py-2 z-10">
        ...
        <div className="flex flex-row justify-between items-center w-full">
          ...
          <div className="flex flex-row items-center">
            {filterButtons.map((f) => {
              ...
              const buttonClass = getFilterButtonClass(f.name, openFilter, label);
              return (
                <div className="mr-2 relative" key={f.name}>
                  ...
                  {isOpen && (
                    <div className="absolute left-0 mt-2 p-3 rounded-lg shadow-lg z-50 min-w-[240px]">
                      {f.component}
                    </div>
                  )}
                </div>
            ...
            {filtersApplied && (
              <button
                className="ml-2 px-3 py-1.5 text-2xs rounded-lg text-neutral-600 dark:text-neutral-300 border border-neutral-400 hover:border-neutral-300 dark:border-neutral-600 dark:hover:border-neutral-500"
          ...
          <div ref={sortButtonRef} className="relative">
            ...
            {openSortFilter && (
              <div className="absolute right-0 mt-2 bg-neutral-100 dark:bg-neutral-800 p-2 rounded-lg shadow-lg z-50 min-w-[120px]">
                <SortFilter
